...
<html>
    ...
    <div class="container">
        ...
        <p>Report shows total fuel drawn for a particular day</p>
        ...
        <div class="card" >
				...
				<div class="card-body">
                    ...
                    <tr>
                        ...
                        $resultset = $mysqli->query("SELECT f.employee_code,f.fn,f.sn,f.dept,f.division,t.fuel_type,sum(t.litres_drawn) as lit FROM mds_transactions t, mds_fuel f WHERE f.employee_code = t.emp_code AND t.date = '$filter'
                        GROUP BY f.employee_code,f.fn,f.sn,f.dept,f.division,t.fuel_type ");
                         if($resultset->num_rows > 0){
                        while($rows=$resultset->fetch_assoc())
                        {
                            $empcode = $rows['employee_code'];$emp = $rows['fn']." ".$rows['sn'];
                            $dept = $rows['dept'];$div = $rows['division'];
                            $lit = $rows['lit'];$ftype = $rows['fuel_type'];

                            echo "<tr><td>$empcode</td><td>$emp</td><td>$dept</td><td>$div</td><td>$ftype</td><td>$lit</td></tr>";
