...
<html>
    ...
    <div class="container">
          ...
          <div class="user"><h3>Month End Report Summary With Cost</h3></div>

        <p>Currently Showing Information For <?php echo substr($_SESSION["filter_date"],0,7); ?></p>
        <p><?php echo "Division ". $_SESSION["division"]; ?></p>
        <div class="card" >
				...
				<div class="card-body">
                    ...
                    <tr>
                        ...
                        $resultset = $mysqli->query("SELECT t.emp_code,fg.fn,fg.sn,fg.division,fg.dept,t.fuel_type,sum(t.litres_drawn) as Litres,round((sum(t.litres_drawn) * p.price),2) as Total_Cost
                        ...
                        and t.emp_code = fg.employee_code
                        AND t.indicator = 'P'

                        GROUP BY t.fuel_type,t.emp_code,fg.fn,fg.sn,fg.division,fg.dept
                        ORDER by fg.fn,fg.sn");
                         if($resultset->num_rows > 0){
                        while($rows=$resultset->fetch_assoc())
                        {
                            $empcode = $rows['emp_code'];$emp = $rows['fn']." ".$rows['sn'];
                            ;$div = $rows['division'];$dept = $rows['dept'];
                            $lit = $rows['Litres'];$ftype = $rows['fuel_type'];
                            $tot_costv=$rows['Total_Cost'];

                            echo "<tr><td>$empcode</td><td>$emp</td><td>$dept</td><td>$div</td><td>$ftype</td><td>$lit</td><td>$tot_costv</td></tr>";
